...
# 1. Cross-validation using input parameters from IPECAD model ----
lifetable_us<- read_csv("lifetable_US_2019.csv") %>% 
  as.data.frame() %>% 
  filter(age>=50&age<=90) %>% 
  rowwise()
  

utilities_ipecad<-c(0.851-0.17,0.851-0.22,0.851-0.36,0.851-0.53,
                     0.851-0.17,0.851-0.19,0.851-0.42,0.851-0.59)

# create a customized "run_model" function to run the HE model
ipecad_runmodel_cust<-function(rx_cycles, 
...
{param <- define_parameters(
  ...
  utilities_mci=utilities_ipecad[1], 
  utilities_mild=utilities_ipecad[2], 
  utilities_mod=utilities_ipecad[3], 
  utilities_sev=utilities_ipecad[4], 
  utilities_mci_inst=utilities_ipecad[5], 
  utilities_mild_inst=utilities_ipecad[6], 
  utilities_mod_inst=utilities_ipecad[7], 
  utilities_sev_inst=utilities_ipecad[8], 
  utilities_death=0, 
  ...
  prob_sev_ariae=0.005,
  
  prob_mild_ariae=0.0135,
  ...
  prob_sev_ariah=0.0035,
  
  prob_mild_ariah=0.0035,
  ...
  age_tv=age_init+(model_time-1)*cycle_length,
  tp_death=lifetable_us$total[(age_tv-50)+1]*cycle_length,
  
  p.mci_mil = 0.23*cycle_length, 
  p.mci_mod = 0, 
  p.mci_sev = 0, 
  p.mil_mci = 0.03*cycle_length, 
  p.mil_mod = 0.35*cycle_length, 
  p.mil_sev = 0.04*cycle_length, 
  p.mod_mil = 0.03*cycle_length, 
  p.mod_sev = 0.42*cycle_length, 
  p.sev_mil = 0, 
  p.sev_mod = 0.02*cycle_length, 
  p.mci_i = 0.024*cycle_length, 
  p.mil_i = 0.038*cycle_length, 
  p.mod_i = 0.110*cycle_length, 
  p.sev_i = 0.259*cycle_length
...
MCI_inst = define_state(
  ...
  *cycle_length*disc_health, 
  ...
  cost = ((cost_mci[2]*cycle_length+cost_admin)*(1-prob_mild_aria-prob_severe_aria-prob_irr)+
            ...
            (cost_mci[2]*cycle_length+cost_admin+cost_irr)*prob_irr)*disc_cost, # cost/cycle = yearly cost/(no.cycles/year)
...
Mild_inst = define_state(
  ...
  *cycle_length*disc_health, 
  ...
  cost = ((cost_ad[4]*cycle_length+cost_admin)*(1-prob_mild_aria-prob_severe_aria-prob_irr)+
            ...
            (cost_ad[4]*cycle_length+cost_admin+cost_irr)*prob_irr)*disc_cost, # cost/cycle = yearly cost/(no.cycles/year)
...
transmat<- define_transition(
  ...
  C, p.mci_mil*rX, 0,     0,     p.mci_i, 0,      0,      0,      tp_death*1.82,
  p.mil_mci, C,     p.mil_mod*rX, p.mil_sev*rX,     0,  p.mil_i,     0,      0,      tp_death*2.92, 
  0, p.mod_mil,     C,     p.mod_sev,    0,  0,       p.mod_i,     0,      tp_death*3.85, 
  0, 0,     p.sev_mod,     C,     0,  0,      0,      p.sev_i ,    tp_death*9.52,
  0, 0,     0,     0,     C,  p.mci_mil, 0,      0,      tp_death*1.82,
  0, 0,     0,     0,     p.mil_mci,  C,      p.mil_mod, p.mil_sev,      tp_death*2.92,
  0, 0,     0,     0,     0,  p.mod_mil,      C,      p.mod_sev,    tp_death*3.85, 
  0, 0,     0,     0,     0,  0,      p.sev_mod,      C,      tp_death*9.52,
  0, 0,     0,     0,     0,  0,      0,      0,      C
)
...
init_state<-c(0.55, 0.45, 0, 0, 0, 0, 0, 0, 0)
...
temp_model_ipecad<-ipecad_runmodel_cust(rx_cycles=10,rx_rr=0.69,
                                     age_init = 71,
                                     wane_fc=1,
...
temp_model_ipecad$eval_strategy_list$standard$counts %>%
  mutate(time = cycle_length * (row_number() - 1),
         MCI=MCI+MCI_inst,
         Mild=Mild+Mild_inst,
...
temp_model_ipecad$eval_strategy_list$rx$counts %>%
  mutate(time = cycle_length * (row_number() - 1),
         MCI=MCI+MCI_inst,
         Mild=Mild+Mild_inst,
         Moderate=Moderate+Moderate_inst,
...
# 2. Cross-validation using similar patient characteristics ----
...
init_state<-c(0.5, 0.5, 0, 0, 0, 0, 0, 0, 0)
