...
ce_summary_excel_func<-function(summary){
  results_excel<-summary %>% 
    select(rx_cycles,rxtime,age,sex,apoe,stage,qaly_soc:costs_rx,deffect,dcost) %>% 
...
# 5.2. Generate plots for sojourn time and transition probability comparisons ----
...
cost_cross_year<-(costplot %>% 
  filter(group=="SoC") %>% 
  rename(cost_soc=cost) %>%
  select(-group) %>% 
  left_join(costplot %>% filter(group=="Treatment")) %>% 
  rename(cost_rx=cost) %>%
  select(-group) %>% 
  filter(cost_rx<cost_soc) %>% 
  slice_min(years))$years

pdf(file = "costs_overtime.pdf",height = 6,width = 8)
ggplot(aes(y=cost, x=years, color=group), data=costplot) + 
  geom_line()+
  geom_vline(xintercept = cost_cross_year,color="grey",linetype="dashed")+
  geom_point()+
  scale_x_continuous(breaks = seq(0, 10, by = 1), expand = c(0, 0)) +
