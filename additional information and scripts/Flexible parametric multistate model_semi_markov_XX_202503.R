...
for (i in 1:13){
  ...
  # run a Cox model
  model<-coxph(Surv(time,status) ~ 1, data=newdata, method="breslow")
...
for (j in 1:6){
  model<-tryCatch({
    flexsurvspline(Surv(time,status) ~ 1,
...
for (i in 7:13){
  ...
  # model with 0 knots
  model0<-flexsurvspline(Surv(time,status) ~ 1,
...
# transition 2, 5, 6, 7, 8, 9, 12, 13 does not fit very well
...
# for transition 5, choose 5 knots
cumhaz_ci_trans5<-ggplot() +
  geom_line(data = nonpara_cumhaz_combined %>% 
              filter(trans==5,
                     time<=10), 
            aes(x = time, y = cumhaz), 
            color = "black") +
  geom_line(data = get(paste0("RP_cumhaz_trans",5,"_with_",5,"knots")), 
            aes(x = .time, y = .pred_cumhaz),color="#56B4E9") +
  geom_ribbon(data = get(paste0("RP_cumhaz_trans",5,"_with_",5,"knots")), 
              aes(x = .time, 
                  ymin = .pred_lower, 
                  ymax = .pred_upper), 
              alpha = .1,fill="#56B4E9") +
  ...
  theme_minimal() +
  theme(plot.title = element_text(size=10)) +
  labs(title="From moderate AD to institutionalized moderate AD",
       x = 'Time (years)',
       y = 'Cumulative hazard',
       linetype = 'Distribution',
       color = "Transition")
...
final_model<-final_model %>% 
  mutate(knots=ifelse(trans==5,5,knots))



# for transition 6, choose 2 knots
cumhaz_ci_trans6<-ggplot() +
  geom_line(data = nonpara_cumhaz_combined %>% 
              filter(trans==6,
                     time<=10), 
            aes(x = time, y = cumhaz), 
            color = "black") +
  geom_line(data = get(paste0("RP_cumhaz_trans",6,"_with_",2,"knots")), 
            aes(x = .time, y = .pred_cumhaz),color="#56B4E9") +
  geom_ribbon(data = get(paste0("RP_cumhaz_trans",6,"_with_",2,"knots")), 
              aes(x = .time, 
                  ymin = .pred_lower, 
                  ymax = .pred_upper), 
              alpha = .1,fill="#56B4E9") +
  ...
  theme_minimal() +
  theme(plot.title = element_text(size=10)) +
  labs(title="From moderate AD to death",
       x = 'Time (years)',
       y = 'Cumulative hazard',
       linetype = 'Distribution',
       color = "Transition")
...
final_model<-final_model %>% 
  mutate(knots=ifelse(trans==6,2,knots))


# for transition 7, choose 5 knots
cumhaz_ci_trans7<-ggplot() +
  geom_line(data = nonpara_cumhaz_combined %>% 
              filter(trans==7,
                     time<=10), 
            aes(x = time, y = cumhaz), 
            color = "black") +
  geom_line(data = get(paste0("RP_cumhaz_trans",7,"_with_",5,"knots")), 
            aes(x = .time, y = .pred_cumhaz),color="#56B4E9") +
  geom_ribbon(data = get(paste0("RP_cumhaz_trans",7,"_with_",5,"knots")), 
              aes(x = .time, 
                  ymin = .pred_lower, 
                  ymax = .pred_upper), 
              alpha = .1,fill="#56B4E9") +
  ...
  theme_minimal() +
  theme(plot.title = element_text(size=10)) +
  labs(title="From severe AD to institutionalized severe AD",
       x = 'Time (years)',
       y = 'Cumulative hazard',
       linetype = 'Distribution',
       color = "Transition")
...
final_model<-final_model %>% 
  mutate(knots=ifelse(trans==7,5,knots))
...
# for transition 9, refit the model
refit_trans9<-flexsurvspline(Surv(time,status) ~ 1,
               data=mstatedata %>% 
                 filter(trans==9&time<=9), k=6,scale="hazard",
               timescale="log")

refit_trans9_cumhaz<-(predict(refit_trans9, type = "cumhaz", newdata = data.frame(AGE = NA),
         times = seq(0, 10, by = 0.1),conf.int=T))[[1]] %>% as.data.frame() %>% 
  rename(.time=.eval_time)

cumhaz_ci_trans9<-ggplot() +
  geom_line(data = nonpara_cumhaz_combined %>% 
              filter(trans==9,
                     time<=10), 
            aes(x = time, y = cumhaz), 
            color = "black") +
  geom_line(data = refit_trans9_cumhaz, 
            aes(x = .time, y = .pred_cumhaz),color="#56B4E9") +
  geom_ribbon(data = refit_trans9_cumhaz, 
              aes(x = .time, 
                  ymin = .pred_lower, 
                  ymax = .pred_upper), 
              alpha = .1,fill="#56B4E9") +
  ...
  theme_minimal() +
  theme(plot.title = element_text(size=10)) +
  labs(title="From institutionalized mild AD to institutionalized moderate AD",
       x = 'Time (years)',
       y = 'Cumulative hazard',
       linetype = 'Distribution',
       color = "Transition")
...
# for transition 12, refit the model
refit_trans12<-flexsurvspline(Surv(time,status) ~ 1,
                             data=mstatedata %>% 
                               filter(trans==12&time<=9), k=4,scale="hazard",
                             timescale="log")

refit_trans12_cumhaz<-(predict(refit_trans12, type = "cumhaz", newdata = data.frame(AGE = NA),
                              times = seq(0, 10, by = 0.1),conf.int=T))[[1]] %>% as.data.frame() %>% 
  rename(.time=.eval_time)


cumhaz_ci_trans12<-ggplot() +
  geom_line(data = nonpara_cumhaz_combined %>% 
              filter(trans==12,
                     time<=10), 
            aes(x = time, y = cumhaz), 
            color = "black") +
  geom_line(data = refit_trans12_cumhaz, 
            aes(x = .time, y = .pred_cumhaz),color="#56B4E9") +
  geom_ribbon(data = refit_trans12_cumhaz, 
              aes(x = .time, 
                  ymin = .pred_lower, 
                  ymax = .pred_upper), 
              alpha = .1,fill="#56B4E9") +
  ...
  theme_minimal() +
  theme(plot.title = element_text(size=10)) +
  labs(title="From institutionalized moderate AD to death",
       x = 'Time (years)',
       y = 'Cumulative hazard',
       linetype = 'Distribution',
       color = "Transition")
...
# for transition 13, refit the model
refit_trans13<-flexsurvspline(Surv(time,status) ~ 1,
                              data=mstatedata %>% 
                                filter(trans==13&time<=9), 
                              knots = c(log(1),log(4),log(5),log(7.5)),scale="hazard",
                              timescale="log")

refit_trans13_cumhaz<-(predict(refit_trans13, type = "cumhaz", newdata = data.frame(AGE = NA),
                               times = seq(0, 10, by = 0.1),conf.int=T))[[1]] %>% as.data.frame() %>% 
  rename(.time=.eval_time)


cumhaz_ci_trans13<-ggplot() +
  geom_line(data = nonpara_cumhaz_combined %>% 
              filter(trans==13,
                     time<=10), 
            aes(x = time, y = cumhaz), 
            color = "black") +
  geom_line(data = refit_trans13_cumhaz, 
            aes(x = .time, y = .pred_cumhaz),color="#56B4E9") +
  geom_ribbon(data = refit_trans13_cumhaz, 
              aes(x = .time, 
                  ymin = .pred_lower, 
                  ymax = .pred_upper), 
              alpha = .1,fill="#56B4E9") +
  ...
  theme_minimal() +
  theme(plot.title = element_text(size=10)) +
  labs(title="From institutionalized severe AD to death",
       x = 'Time (years)',
       y = 'Cumulative hazard',
       linetype = 'Distribution',
       color = "Transition")
...
for (i in c(1:8,10:11)){
  ...
  assign(paste0("final_model_trans",i),
         flexsurvspline(formula = Surv(time,status) ~ AGE+SEX, data = newdata, 
                        k = k, scale = "hazard",
...
final_model_trans9<-flexsurvspline(Surv(time,status) ~ AGE+SEX,
                                   data=mstatedata %>% 
                                     filter(trans==9), k=6,scale="hazard")


final_model_trans12<-flexsurvspline(Surv(time,status) ~ AGE+SEX,
                                   data=mstatedata %>% 
                                     filter(trans==12), k=4,scale="hazard")


final_model_trans13<-flexsurvspline(Surv(time,status) ~ AGE+SEX,
                                    data=mstatedata %>% 
                                      filter(trans==13), 
                                    knots = c(log(1),log(4),log(5),log(7.5)),scale="hazard")
...
pdf(file="model fit_semi markov.pdf",width = 12,height = 14)
...
save(list = c(paste0('final_model_trans', 1:13),"mci_ad_model","mci_death_model"), 
     file = 'semi_markov_models_xx.RData')
