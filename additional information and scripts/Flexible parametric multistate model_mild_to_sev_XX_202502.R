...
# ************************************************************************************************
load("mstatedata_xx_mild_to_sev.RData")
...
nonpara_cumhaz_combined<-do.call(rbind, lapply(1:15, function(i) {
...
rp_cumhaz_combined <- do.call(rbind, lapply(1:15, function(i) {
...
model_fitness<-do.call(rbind, lapply(c(1:15), function(i) {
  do.call(rbind, lapply(1:5, function(k) {
...
ggarrange(plot_trans13,plot_trans14,plot_trans15,ncol = 3,nrow = 1,common.legend = T)
...
ggarrange(cumhaz_ci_trans1,cumhaz_ci_trans2,cumhaz_ci_trans3,ncol = 3,nrow = 1,common.legend = T)
ggarrange(cumhaz_ci_trans4,cumhaz_ci_trans5,cumhaz_ci_trans6,ncol = 3,nrow = 1,common.legend = T)
ggarrange(cumhaz_ci_trans7,cumhaz_ci_trans8,cumhaz_ci_trans9,ncol = 3,nrow = 1,common.legend = T)
ggarrange(cumhaz_ci_trans10,cumhaz_ci_trans11,cumhaz_ci_trans12,ncol = 3,nrow = 1,common.legend = T)
ggarrange(cumhaz_ci_trans13,cumhaz_ci_trans14,cumhaz_ci_trans15,ncol = 3,nrow = 1,common.legend = T)

# transition 2 does not fit very well

# 3. Deal with models that did not fit well ----
# for transition 2, 6 knots fit better
cumhaz_ci_trans2<-ggplot() +
  geom_line(data = nonpara_cumhaz_combined %>% 
            ...
            aes(x = time, y = cumhaz), 
            color = "black") +
  geom_line(data = RP_cumhaz_trans2_with_6knots, 
            aes(x = .time, y = .pred_cumhaz),color="#56B4E9") +
  geom_ribbon(data = RP_cumhaz_trans2_with_6knots, 
              aes(x = .time, 
                  ymin = .pred_lower, 
                  ymax = .pred_upper), 
              alpha = .1,fill="#56B4E9") +
  ...
  theme_minimal() +
  theme(plot.title = element_text(size=10)) +
  labs(title = "From mild AD to severe AD",
       x = 'Time (years)',
       y = 'Cumulative hazard',
       linetype = 'Distribution',
       color = "Transition")
...
final_model<-final_model %>% 
  mutate(knots=ifelse(trans==2,6,knots))
...
ggarrange(cumhaz_ci_trans1,cumhaz_ci_trans2,cumhaz_ci_trans3,
          ...
          cumhaz_ci_trans13,cumhaz_ci_trans14,cumhaz_ci_trans15,
...
pdf(file="model fit_mild_to_sev.pdf",width = 12,height = 14)
...
save(list = c(paste0('final_model_trans', 1:15),"mci_ad_model","mci_death_model"), 
     file = 'models_xx_mild_to_sev.RData')
