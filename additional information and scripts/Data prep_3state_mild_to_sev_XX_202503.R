...
# restric to 2014-01-01 to 2020-12-31 to load less data
...
# few people (<5) transit from mild to institutionalized moderate, replace to moderate
TRANS_INST<-TRANS_INST %>% 
  mutate(next_state=ifelse(state==1&next_state==5,2,next_state),
         next_state=ifelse(state==2&next_state==6,3,next_state))

TRANS_INST<-TRANS_INST %>% 
  filter(!DATE==next_date) %>% 
  filter(next_state!=99) %>% 
  group_by(LOPNR,state) %>% 
...
attributes(mstatedata)$trans<-transMat(x=list(c(2,3,4,7), 
                                              c(3,5,7),
                                              c(6,7),
                                              c(5,6,7),
                                              c(6,7),
...
save(list=c('costwide','mci_svedem_costs'), file='costdata_xx_mild_to_sev.RData')
save(list=c('mstatedata', 'eventdata','nacc_mci_select'), file='mstatedata_xx_mild_to_sev.RData')  
